...
const fetchCourseDetails = async (req, res) => {
  ...
  const course = await Course.populate(queryResult, {
...
const fetchAllSearchedCorses = async (req, res) => {
  const { search, courseLanguage, courseLevel, rating, sort } = req.query;
  const queryObject = {
    courseTitle: { $regex: search, $options: "i" },
  };
...
const fetchCoursesByCategory = async (req, res) => {
  ...
  const { courseLanguage, courseLevel, rating, sort, category } = req.query;
